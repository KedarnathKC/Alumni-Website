...
<script type="text/javascript">
    function ValidateChangePasswordForm(){
            ...
            var oldPass=document.ChangePasswordForm.opass.value;
            var NewPass1=document.ChangePasswordForm.npass1.value;
            var NewPass2=document.ChangePasswordForm.npass2.value;
           
            regExpPass=/^[A-Za-z0-9@$]{6,16}$/;
            
            if(!oldPass.match(regExpPass) || !NewPass1.match(regExpPass) ||!NewPass2.match(regExpPass))
            {
                //document.getElementById("pass").innerHTML="Please Enter Valid Password";
                alert("Please Enter Valid Password. Min 6 characters and Max 16 characters are allowed");
                return false;
            }
...
<?php
    ...
    if(isset($_SESSION['uid']))
    {
        ...
        if(isset($_POST['forgot'])){
            ...
            $get_original="select * from credentials where id=$uid";
            $result=mysqli_query($dbhandle,$get_original);
            if($result){
                $dbpass=mysqli_fetch_assoc($result)['password'];
                if(strcmp($opass, $dbpass)==0){
                    if(strcmp($npass1,$npass2)==0){
                        $update="update credentials set password='$npass1' where id=$uid";
        ...
        <div class="register-page-wrapper section-padding">
            <div class="container">
                <div class="row">
                    ...
                    <form name = "ChangePasswordForm" onsubmit="return ValidateChangePasswordForm()" action="ChangePassword.php" method="POST" ><!--action="https://codeboxr.net/themedemo/unialumni/html/index.html"-->
                        <input type="password" placeholder="Current Password" name="opass" data-placement="bottom" data-tooltip title="Enter Current Password" required><br><br>
                        <input type="password" placeholder="New Password" name="npass1" data-placement="bottom" data-tooltip title="Enter New Password" required><br><br>
                        <input type="password" placeholder="New Password" name="npass2" data-placement="bottom" data-tooltip title="Re-type New Password" required><br><br>
                        <center><button type="submit" class="btn btn-reg" name="forgot">Submit</button></center><br>
                    </form>
