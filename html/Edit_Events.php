<?php
    ...
    if(isset($_POST["update"])){
        ...
        $event_desc=$_POST['edesc'];
        $event_location=$_POST['elocation'];
        $event_time=$_POST['etimestamp'];
        //$id = $_GET['eid'];
        //echo "<script> alert('$event_name')</script>";
        $update="Update event_desc set event_name = '$event_name', event_location = '$event_location', event_description = '$event_desc' , event_timestamp = '$event_time' where event_id = '$id'";
        //$result=mysqli_query($dbhandle,$update) or die(mysqli_error($dbhandle));
        if(!mysqli_query($dbhandle,$update)){
            echo mysqli_error($dbhandle);
            echo "<script>alert('Unable to Update')</script>";
            echo "<script> window.location.href='Edit_Events.php'</script>";   
        }
        else{
            echo "<script>alert('Successfull');</script>";
            echo "<script>window.location.href='Event_Details.php'</script>";
        }
    ...
    $sql = "select *,DATE_FORMAT(event_timestamp, '%Y-%m-%dT%H:%i') AS custom_date from event_desc where event_id = '$id'";
    $result_set=mysqli_query($dbhandle,$sql);
   
        $row=mysqli_fetch_assoc($result_set);
        $ename = $row['event_name'];
        //echo "<script>alert('$ename')</script>";   
        $edesc = $row['event_description'];
        $eloc = $row['event_location'];
        //echo "<script>alert('$edesc')</script>"; 
        $etime = $row['custom_date'];
        //echo "<script>alert('$etime')</script>";
                                ...
                                <div class="register-form-content">
                                    <div class="row">                                            
                                            <div class="" style="width: 90%;">
                                                <h3 style="text-align: center;margin-left: 10%;">Event Page</h3>
                                                <div class="register-form" style="width: 50%;margin-left: 30%;">
                                                    <form name="EventForm" onsubmit="return ValidateEventForm()" action="Edit_Events.php?eid=<?php echo $id; ?>" method="POST">
                                                      <div class="form-group">
                                                        <label for="eventName">Event Name</label>
                                                        <?php 
                                                        echo "<input type='text' class='form-control' id='eventName' name='ename' value='".$ename."' placeholder='Enter Event Name' data-tooltip title='Max length is 50' required>";
                                                       ?>
                                                      </div>
                                                      <div class="form-group">
                                                        <label for="eventLocation">Event Location</label>
                                                        <?php
                                                        echo '<input type="text" class="form-control" id="eventLocation" name="elocation" value = "'.$eloc.'"  placeholder="Enter Event Location" data-tooltip title="Max length is 50" required>';
                                                      ...
                                                      <div class="form-group">
                                                        ...
                                                        <?php
                                                        echo "<input type='datetime-local' class='form-control' id='eventTimestamp' name='etimestamp' value = '".$etime."' placeholder='Enter Event Date and Time'  required>";
                                                       ?>
                                                      </div>
                                                      <div class="form-group">
                                                        <label for="eventName">Event Description</label>
                                                        <?php
                                                        echo '<textarea rows="3" name="edesc" class="form-control" id="eventName"  placeholder="Enter Event Description" data-tooltip title="Max length is 250" required>'.$edesc.'</textarea>';
                                                       ?>
                                                      </div>
                                                      <div class="form-group">
                                                       <center> <input class = "btn-auth btn-auth-rev" type="submit" name = "update" value="update"></center>                                                     
                                                        </div>
                                                        </div>
                                                    </form>
                                                </div>
