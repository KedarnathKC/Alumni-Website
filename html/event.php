<?php
    ...
    $get_registered="select event_id from event_info where alumni_id=$uid";
    $get_result=mysqli_query($dbhandle,$get_registered);
    $registered=array();
    while($row1=@mysqli_fetch_assoc($get_result)){
        array_push($registered,$row1['event_id']);
    }  
?>
                ...
                <div class="col-lg-12">
                    <nav class="main-menu navbar navbar-expand-lg navbar-light">
                        ...
                        <div class="collapse navbar-collapse" id="menucontent">
                                ...
                                <li class="nav-item active"><a class="nav-link" href="event.php">Event</a></li>
                            ...
                            <div class="col-lg-10 m-auto">
                                <div class="register-form-content">
                                    <div class="row">
                                        ...
                                        <?php
                                            $get_events="SELECT * FROM event_desc where event_timestamp > NOW(); ";
                                            $result=mysqli_query($dbhandle,$get_events);

                                            if(@mysqli_num_rows($result)>0){
                                                while($row=mysqli_fetch_assoc($result)){
                                                    echo "<form method='POST' action='event_register.php'>";
                                                        echo "<input type='hidden' name='eid' value='".$row["event_id"]."'>";
                                                        //echo "<div style = 'color:white;border:2px solid;text-align:center;background:url(Background2.jpg);'>";
                                                        echo "<div style='text-align:center;height:400px;color:white;border: solid 2px black;background: url(Background4.jpg);opacity: 1.0;'>";
                                                        echo "<h6>Name :".$row["event_name"]."</h6><br>";
                                                        echo "<input type='hidden' name='ename' value='".$row["event_name"]."'>";
                                                        //echo "<h6>Date :".$row["event_date"]."</h6><br>";
                                                        echo "<h6>Location :".$row["event_location"]."</h6><br>";
                                                        echo "<input type='hidden' name='eloc' value='".$row["event_location"]."'>";
                                                        echo "<h6>Time :".$row["event_timestamp"]."</h6><br>";
                                                        echo "<input type='hidden' name='etime' value='".$row["event_timestamp"]."'>";
                                                        echo "<h6>Description :".$row["event_description"]."</h6><br>";
                                                        echo "<input type='hidden' name='edesc' value='".$row["event_description"]."'>";
                                                        
                                                        if(in_array($row["event_id"],$registered))
                                                            echo "<input type='button' name='event_reg' class='btn-auth btn-auth'value='Registered' readonly>";
                                                        else
                                                            echo "<input class='btn-auth btn-auth' type='Submit' name='event_reg' value='Interested'>";
                                                    echo "</div>";
                                                    echo "</form>";
                                                ...
                                                echo "<br><h4> No Events scheduled currently to show</h4>";
                                            }  
                                        ?> 
                                        </div> 
                                        </div>
